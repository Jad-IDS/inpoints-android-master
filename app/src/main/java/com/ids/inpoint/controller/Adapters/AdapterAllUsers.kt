...
class AdapterAllUsers(val arrayUsers: ArrayList<ResponseUser>, private val itemClickListener: RVOnItemClickListener,type:Int) :
    RecyclerView.Adapter<AdapterAllUsers.VHprivacy>() {

    var userType=type

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): VHprivacy {
        return VHprivacy(LayoutInflater.from(parent.context).inflate(com.ids.inpoint.R.layout.item_user, parent, false))
    }
    ...
    inner class VHprivacy(itemView: View) : RecyclerView.ViewHolder(itemView), View.OnClickListener {
       ...
       else if(userType==AppConstants.PICK_IMAGE_INVESTOR)
           itemView.id=R.id.linearItemUserInvestor
       else if(userType==AppConstants.PICK_IMAGE_SPONSOR)
           itemView.id=R.id.linearItemUserSponsor
